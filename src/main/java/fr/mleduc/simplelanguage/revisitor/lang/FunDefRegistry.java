...
/**
 * Manages the mapping from function names to {@link FunDef function objects}.
 */
public final class FunDefRegistry {

    private final SLRLanguage language;
    private final FunctionsObject functionsObject = new FunctionsObject();

    public FunDefRegistry(SLRLanguage language) {
        this.language = language;
    }

    /**
     * Returns the canonical {@link FunDef} object for the given name. If it does not exist yet,
     * it is created.
     */
    ...
    /**
     * Associates the {@link FunDef} with the given name with the given implementation root
     ...
     */
    public List<FunDef> getFunctions() {
        List<FunDef> result = new ArrayList<>(functionsObject.functions.values());
        Collections.sort(result, new Comparator<FunDef>() {
            public int compare(FunDef f1, FunDef f2) {
